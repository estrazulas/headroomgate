...
`headroom audit` Click command group for querying usage audit and analytics.
...
- **WHEN** admin runs `headroom audit user alice --last 7d`
...
- **WHEN** admin runs `headroom audit user alice --last 7d --by-day`
...
- **WHEN** admin runs `headroom audit user alice --last 7d --by-model`
