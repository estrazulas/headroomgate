...
In-memory TTL-based validation cache that avoids querying Neo4j on every request, with stale-entry fallback when Neo4j is unreachable.

## Purpose

Cache resolved user identities in memory to minimize Neo4j query volume (~100x reduction: 1 query per key per 10s instead of 1 per request). Provider keys are decrypted once at cache time. When Neo4j is temporarily unreachable, expired (stale) entries are served as a fallback so the proxy continues operating. The TTL is configurable via `HEADROOM_AUTH_CACHE_TTL`.
...
- **THEN** Neo4j is queried via `Neo4jAuthStore.resolve_key_identity()`
