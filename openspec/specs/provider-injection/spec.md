...
Automatic upstream LLM provider resolution by request path and real API key injection using Fernet-decrypted provider keys.

## Purpose

Determine which upstream LLM provider to use based on the request path (e.g., `/v1/messages` → anthropic), look up the encrypted provider key from the user's role, decrypt it via `FernetCrypto` from PRD 1, and replace the proxy `Authorization` header with the real provider key before forwarding to the upstream.
