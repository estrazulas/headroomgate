...
Symmetric encryption (Fernet) for protecting provider API keys at rest in Neo4j.

## Purpose

Ensure that LLM provider API keys (Anthropic, OpenAI, Gemini, etc.) stored in the `Role.provider_keys` property are never persisted in plaintext. Fernet provides AES-128-CBC + HMAC-SHA256 with built-in timestamp validation, making it safe-by-default for the "encrypt at rest" use case.
...
### Requirement: Provider key validation
The system SHALL validate a provider API key at storage time by making a lightweight test request to the provider's API before encrypting and storing. Invalid keys SHALL be rejected immediately.

#### Scenario: Valid key is stored
- **WHEN** admin runs `headroom auth set-provider-key developer anthropic` with a valid key
- **THEN** the system makes a test request to the Anthropic API, confirms validity, and stores the encrypted key

#### Scenario: Invalid key is rejected
- **WHEN** admin runs `headroom auth set-provider-key developer anthropic` with an invalid key
- **THEN** the system returns an error "Error: anthropic API key is invalid or has insufficient permissions"
