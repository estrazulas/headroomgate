...
ASGI middleware that authenticates every incoming request by validating `hr_...` proxy keys against Neo4j, resolving user identity, and propagating it via contextvars.

## Purpose

Provide the runtime authentication layer for the headroom proxy. The middleware is packaged as a `headroom.proxy_extension` plugin at `plugins/headroom-auth/` and is opt-in — when not activated, the proxy operates identically to the original. Health check endpoints (`/livez`, `/readyz`, `/health`, `/metrics`) never require authentication. When `HEADROOM_AUTH_ENABLED=false`, all requests pass through without validation.
