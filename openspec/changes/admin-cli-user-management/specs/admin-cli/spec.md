...
Interface de linha de comando (`headroom auth`) para administração de usuários, roles, times, API keys e provider keys.

## ADDED Requirements

### Requirement: CLI group registration
The system SHALL register `headroom auth` as a Click command group under the main `headroom` CLI. The group SHALL be available when the `headroom-ai` package is installed.
...
The CLI SHALL enforce access control based on the identity of the user executing the command. Identity is resolved from the `HEADROOM_AUTH_USER` environment variable (set by the operator's shell profile) or the `--as-user` flag.
